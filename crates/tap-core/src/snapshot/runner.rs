...
//!
//! Exports a Postgres snapshot via `pg_export_snapshot()`, then scans each
//! table in order, emitting one `op:'r'` (Read) [`ChangeEvent`] per row and
//! checkpointing progress to the [`StateStore`].
...
use crate::postgres::{Lsn, PgConnection};
...
struct TableInfo {
    ...
    name: String,
    /// Schema-qualified name for SQL (`"schema.table"`).
    qualified: String,
}
...
///
/// Uses `pg_export_snapshot()` to obtain a consistent cross-table snapshot,
/// then scans each table one-by-one, emitting `Read` events and checkpointing
/// progress into the state store.
///
/// # Errors
///
...
///
/// let runner = SnapshotRunner::new(pg_conn, state, config, event_tx);
/// let result = runner.run().await?;
/// println!("snapshotted {} rows from {} tables",
///     result.total_rows, result.tables_snapshotted.len());
/// ```
pub struct SnapshotRunner {
    /// Wrapped Postgres connection (client for direct SQL queries).
    pg_conn: PgConnection,
    /// Shared state store (SQLite), behind a mutex for thread-safety.
...
impl SnapshotRunner {
    ...
    ///
    /// 1. Exports a snapshot via `pg_export_snapshot()` and records the LSN.
    /// 2. Discovers tables (from config or the publication).
    /// 3. Scans each table emitting `Read` events.
    /// 4. Returns a summary [`SnapshotResult`].
    pub async fn run(&self) -> Result<SnapshotResult, TapError> {
        ...
        for table in &tables {
            ...
            total_rows += rows;
            tables_snapshotted.push(table.qualified.clone());
        }
    ...
    async fn export_snapshot(&self) -> Result<(String, Lsn), TapError> {
        let client = self.pg_conn.client();

        // BEGIN READ ONLY REPEATABLE READ — establishes the snapshot point
        client
    ...
    async fn resolve_tables(&self) -> Result<Vec<TableInfo>, TapError> {
        if !self.config.tables.is_empty() {
            let mut tables: Vec<TableInfo> = self
                ...
                .collect();
            // Sort by schema then table name for deterministic order
            tables.sort_by(|a, b| a.qualified.cmp(&b.qualified));
            return Ok(tables);
        }

        // Discover all tables in the publication
        let pub_name = &self.pg_conn.config().publication;
        let client = self.pg_conn.client();
    ...
    // -----------------------------------------------------------------------
    // Step 3: Snapshot a single table
    // -----------------------------------------------------------------------

    /// Snapshot one table: detect PK, scan rows, emit events, checkpoint.
    async fn snapshot_table(
        ...
        let client = self.pg_conn.client();
        let db_name = self.pg_conn.config().dbname.clone();
        ...
        client.simple_query(&set_snap).await?;

        // ── Detect primary key columns  ───────────────────────────────
        let pk_columns = self.detect_pk_columns(table).await?;
        ...
        // Use CTID as tiebreaker when PK is not unique (shouldn't happen,
        // but keeps the sort deterministic).
        let query = format!(
            "SELECT * FROM {} ORDER BY {}, ctid",
            table.qualified, order_clause
        );
        ...
        if total == 0 {
            info!("table {} is empty, snapshot complete", table.qualified);
            client.batch_execute("COMMIT").await?;
            self.mark_table_complete(table, snapshot_id, 0)?;
            return Ok(0);
        }
        ...
        for (i, row) in rows.iter().enumerate() {
            ...
            if (i + 1) % self.config.batch_size as usize == 0 {
                let rows_done = i as u64 + 1;
        ...
        {
            let state = self.state.lock().unwrap();
            state.write_snapshot_progress(&table.qualified, snapshot_id, total, snapshot_lsn)?;
        }

        // Commit the table transaction
        client.batch_execute("COMMIT").await?;

        // Mark completed
        self.mark_table_complete(table, snapshot_id, total)?;
    ...
    async fn detect_pk_columns(&self, table: &TableInfo) -> Result<Vec<String>, TapError> {
        let client = self.pg_conn.client();
    ...
    fn is_table_completed(&self, table: &TableInfo) -> Result<bool, TapError> {
        let state = self.state.lock().unwrap();
    ...
    fn completed_row_count(&self, table: &TableInfo) -> Result<u64, TapError> {
        let state = self.state.lock().unwrap();
        match state.get_snapshot_status(&table.qualified)? {
...
mod tests {
    ...
    /// Build a minimal `Row`-like structure for testing is hard without a
    /// live connection.  These tests verify the JSON construction logic
    /// that wraps the result of `try_get`.
    ///
    /// We test the helper that builds a JSON map from column names and
    /// their `serde_json::Value` representations.
    #[test]
    fn test_build_snapshot_event_id_single_pk() {
        // We can't easily create a real `Row` without Postgres, so we test
        // the ID *format* logic assuming correct values.
    ...
    fn test_build_snapshot_event_id_composite_pk() {
        ...
        // Composite PK format: key=value:key=value
        let parts = pk_cols
            .iter()
    ...
    fn test_row_to_json_basic_types() {
        // Verify the JSON map structure we build from column data
    ...
    fn test_parse_empty_table_name() {
        // An empty string should give schema="public", name=""
    ...
    fn test_snapshot_source_metadata_flags() {
        // Verify SourceMetadata is correctly configured for snapshot events
        let source = SourceMetadata {
            db: "test_db".into(),
            schema: "public".into(),
            table: "users".into(),
